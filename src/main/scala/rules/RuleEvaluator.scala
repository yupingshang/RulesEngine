...
private[rules] abstract class RuleManipulator(workingMemory: WorkingMemory) {
...
private[rules] class RuleEvaluator(rule: Rule, workingMemory: WorkingMemory)
  ...
  private[rules] def registerExecution(executedSet: RuleExecutionSet) = {
...
private[rules] class RuleExecutor(evaluator: RuleEvaluator, rule: Rule, workingMemory: WorkingMemory, executionSet: RuleExecutionSet)
                  extends RuleManipulator(workingMemory) with Logger {
...
private class ValuesCombinator {
  ...
  private[rules] def hasNext() = !values.isEmpty && !finished

  private[rules] def +=[A] (t: Traversable[A]): Option[A] = {
    values = t.toList :: values
    finished = t.isEmpty
    if (finished) None else Some(t.head)
  }

  private[rules] def next[A](clazz: Class[A]): Option[A] = {
